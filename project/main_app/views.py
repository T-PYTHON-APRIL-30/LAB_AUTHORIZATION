...
def home(request):
    ...
    return render(request ,'home.html', {'clinic_list' : clinic_list ,})
@login_required
def clinic_list(request):
    clinic_list  = Clinic.objects.all()
    return render(request ,'my_clinic_list.html', {'clinic_list' : clinic_list ,'title' : 'كل طلبات الاستشارة', })
@login_required 
def clinic_detail(request , id):
    clinic_detail  = get_object_or_404(Clinic, id=id)
    return render(request ,'clinic_detail.html', {'clinic_detail' : clinic_detail ,})
@login_required 
def add_clinic(request):
    ...
    return render(request ,'add_clinic.html', { 'clinic_form' : clinic_form ,})
@login_required
