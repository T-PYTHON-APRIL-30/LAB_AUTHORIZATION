...
def appointment_list(request):
    ...
    return render(request ,'appointment_list.html', {'appointment_list' : appointment_list ,'title' : 'كل طلبات الاستشارة',})
...
def appointment_list_category(request , department): 
    ...
    if 'appointmendelete' in request.POST :
        if request.user.is_authenticated:
            ...
            appointmenItem= Appointment.objects.get(id=appointmentId)
            appointmenItem.delete()
    return render(request ,'appointment_list_category.html', {'appointments' : appointments ,})
...
def appointment_detail(request , id):
    ...
    return render(request ,'appointment_detail.html', {'appointment_detail' : appointment_detail ,})
...
def add_Appointment(request, id=id):
    ...
    if request.method == 'POST':
        ...
        if appointment_form.is_valid():
            ...
            existing_appointment = Appointment.objects.filter(clinic=clinic,appointment_datetime=appointment.appointment_datetime).first()
